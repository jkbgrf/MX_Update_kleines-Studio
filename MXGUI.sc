...
MXGUI { // singleton !
	...
	classvar <mainPanel, <nearPanel, <phonesPanel, <wfsPanel, <domePanel;
...
//	classvar <>textColor;
	...
	classvar <wfsOn, <wfsVolSlider, <wfsVolNumber, <wfsLimit, <wfsMono, <wfsLeft, <wfsRight, <wfsLPhase, <wfsRPhase, <wfsSwap;
	classvar <mainDelayOn, <domeDelayOn;
	classvar <domeOn, <domeVolSlider, <domeVolNumber, <domeLimit, <domeMono, <domeLeft, <domeRight, <domeLPhase, <domeRPhase, <domeSwap;
...
//		rtaPanel = SCCompositeView(winPanel, Rect(xp[5], yp[3], xp[6]-xp[5], yp[4]-yp[3] ));
	...
	*makeSetupViews {
		...
		setupButtonSize = Point( (setupPanel.bounds.width - (setupPanel.decorator.gap.x * 9)) / 10 - 0.1, 24);
		...
		MXButton(setupPanel, setupButtonSize, radius)
			.shifty_(shifty)
			.states_([ ["Restart Mac", setupButtonTextColor, setupButtonBackColor] ])
			.font_(setupButtonFont)
			.action_({ 
				var win, t;
				win = SCWindow("Do you really want to restart the Matrix computer?", Rect(100, 400, 450, 150));
				win.front;
				t = Task({ inf.do { arg i;  
					win.view.background = Color.red( (i / 30).fold(0.1, 0.8) );
					0.01.wait;
				...
				SCButton(win, Rect(300,50,100,40))
					.font_(Font("Helvetica-Bold", 20))
					.states_([ ["OK", Color.black, Color.red] ])
					.action_({ win.close; MXMain.restart; })
					;
				 })
			;
	...
	*makeMeterViews {
		// 16 meters!  (eigentlich nur 12+1, aber fuer WFS besser mehr ?)  -> MXGlobals.multiMeterChannels ....
	...
	*makeVolumeViews {
		var mwidth, cwidth, theight, bheight, sheight, pgap=8 /* 45 */, sgap = 5, hgap = 9;
	...
	//	mwidth = volumePanel.bounds.width - (hgap*2) - (pgap * 2) / 3 - 1; // button width
		mwidth = volumePanel.bounds.width - (hgap*2) - (pgap * 3) / 4 - 0.0; // button width
	//	cwidth = pgap - 20; // unit / comment width
		...
		wfsPanel  = SCCompositeView(volumePanel, Rect((mwidth + pgap) * 1 + hgap, hgap, mwidth, volumePanel.bounds.height  - hgap));
		wfsPanel.background = panelColor;
		wfsPanel.decorator = FlowLayout(wfsPanel.bounds, margin: 0@0, gap: sgap@hgap);

		domePanel  = SCCompositeView(volumePanel, Rect((mwidth + pgap) * 2 + hgap, hgap, mwidth, volumePanel.bounds.height  - hgap));
		domePanel.background = panelColor;
		domePanel.decorator = FlowLayout(domePanel.bounds, margin: 0@0, gap: sgap@hgap);
/*
...
*/
		phonesPanel  = SCCompositeView(volumePanel, Rect( (mwidth + pgap) * 3 + hgap, hgap, mwidth, volumePanel.bounds.height  - hgap));
...
*/
		...
		// WFS
		wfsOn = MXButton(wfsPanel, monitorTitleSize, corner)
			.shifty_(shifty)
			.states_([  [MXMonitorManager.wfsMonitor.name, setupButtonTextColor, setupButtonBackColor],
					   [MXMonitorManager.wfsMonitor.name, setupButtonTextColor, Color.green] ])
			.font_(setupButtonFont);
		wfsOn.connect(MXMonitorManager.wfsMonitor.active);
		
		// dummy:
		MXButton(wfsPanel, monitorButtonSize, corner).visible_(false);

		setupNameView = SCStaticText(wfsPanel, monitorButtonSize)
			.font_(setupButtonFont)
			.stringColor_(infoTitleTextColor)
			.background_(Color.grey(0.1))
			.align_(\center)
		...
		mainDelayOn = MXButton(wfsPanel, monitorButtonSize, corner)
			.shifty_(shifty)
			.states_([  ["Meyer 1-8", setupButtonTextColor, setupButtonBackColor],
					   ["Meyer 1-8", setupButtonTextColor, plugColor] ])
			.font_(setupButtonFont);
			
		mainDelayOn.connect(MXMonitorManager.mainMonitor.delayOn);


		domeDelayOn = MXButton(wfsPanel, monitorButtonSize, corner)
			.shifty_(shifty)
			.states_([  ["Dome", setupButtonTextColor, setupButtonBackColor],
					   ["Dome", setupButtonTextColor, plugColor] ])
			.font_(setupButtonFont);
			
		domeDelayOn.connect(MXMonitorManager.domeMonitor.delayOn);

		// dummy:
		MXButton(wfsPanel, monitorButtonSize, corner).visible_(false);

	/*
		wfsLimit = MXButton(wfsPanel, monitorButtonSize, corner)
			.shifty_(shifty)
			.states_([  ["limiter", setupButtonTextColor, setupButtonBackColor],
					   ["limiter", setupButtonTextColor, plugColor] ])
			.font_(setupButtonFont);
	*/		
	//	wfsLimit.connect(MXMonitorManager.wfsMonitor.limiter);

		wfsVolNumber = MXNumber(wfsPanel, monitorButtonSize, corner)
			.shifty_(shifty)
			.background_(levelBackColor)
			.stringColor_(levelColor)
			.align_(\center)
			...
			.unit_(" dB")
			;
		wfsVolNumber.connect(MXMonitorManager.wfsMonitor.gain);

		wfsVolSlider = MXSlider(wfsPanel, monitorSliderSize, corner)
			.background_(sliderBackColor)
			.knobColor_(sliderKnobColor)
			.thumbSize_(20)
			;
		wfsVolSlider.connect(MXMonitorManager.wfsMonitor.gain);


		// dome / ambisonics

		domeOn = MXButton(domePanel, monitorTitleSize, corner)
			.shifty_(shifty)
			.states_([  [MXMonitorManager.domeMonitor.name, setupButtonTextColor, setupButtonBackColor],
					   [MXMonitorManager.domeMonitor.name, setupButtonTextColor, Color.green] ])
			.font_(setupButtonFont);
		domeOn.connect(MXMonitorManager.domeMonitor.active);
		

		// dummy:
		MXButton(domePanel, monitorButtonSize, corner).visible_(false);
		...
		MXButton(domePanel, monitorButtonSize, corner).visible_(false);
/*	
		domeLimit = MXButton(domePanel, monitorButtonSize, corner)
			.shifty_(shifty)
			.states_([  ["limiter", setupButtonTextColor, setupButtonBackColor],
					   ["limiter", setupButtonTextColor, plugColor] ])
			.font_(setupButtonFont);
*/			
	//	domeLimit.connect(MXMonitorManager.domeMonitor.limiter);

		domeVolNumber = MXNumber(domePanel, monitorButtonSize, corner)
			.shifty_(shifty)
			.background_(levelBackColor)
			.stringColor_(levelColor)
			.align_(\center)
			...
			.unit_(" dB")
			;
		domeVolNumber.connect(MXMonitorManager.domeMonitor.gain);

		domeVolSlider = MXSlider(domePanel, monitorSliderSize, corner)
			.background_(sliderBackColor)
			.knobColor_(sliderKnobColor)
			.thumbSize_(20)
			;
		domeVolSlider.connect(MXMonitorManager.domeMonitor.gain);
