...
*/	
	...
	var <phase;				// MXCV: phase [-1, 1], default 1 (als Faktor fuer gain)
	var <active; 				// MXCV: [0, 1] 
	...
	*new { arg target, frombus, tobus, gain=0.0, phase=1;
		^super.new.init(target, frombus, tobus, gain, phase);
	}
	
	init { arg argtarget, argfrombus, argtobus, arggain, argphase;
		target = argtarget;
		frombus = argfrombus;
		tobus = argtobus;
		gain = MXCV( ControlSpec(MXGlobals.dbmin, MXGlobals.dbmax, \lin, 0), arggain ? 0.0);
		phase = MXCV( ControlSpec(-1, 1, \lin, 1), argphase ? 1);   
	...
	//	active.action = { arg changer, what; { ["active", frombus.num, tobus.num , changer.value, what].postln  }.defer };
		synth = Synth.controls(defname, [
			in: frombus.busNum, out: tobus.busNum, gain: [[gain, phase, active], gain.dbamp * phase * active]
			], target);
	}
...
*/		
	...
	*new { arg fromBusArray, toBusArray;
	...
	arrayconnect { arg array;
		var from, to, gain = 0.0, phase=1;
		array.do { arg conn, i;
			from = fromBusArray[conn[0] - 1];
			to = toBusArray[conn[1] - 1];
			if ( conn.size > 2 ) { gain = conn[2] };
			if ( conn.size > 3 ) { phase = conn[3] };
			this.addNode( from, to, gain, phase );
		}
	}
	...
	addNode { arg frombus, tobus, gain=0.0, phase=1;
		var node;
		node = MXNode(group, frombus, tobus, gain, phase);
	...
	setNodeByIndex {Êarg from, to, gain, phase;
		var frombus, tobus;
		frombus = fromBusArray[from];
		tobus = toBusArray[to];
		case 
			{ (gain > -90) && nodes[ [frombus, tobus] ].isNil } { 
				this.addNode(frombus, tobus, gain, phase);
				nodeLabels.add( [from, to] -> [inLabels2[from], outLabels2[to], gain, phase] );
				 }
			{ (gain > -90) && nodes[ [frombus, tobus] ].notNil } { 
				nodes[ [frombus, tobus] ].gain_(gain);
				nodes[ [frombus, tobus] ].phase_(phase);
				nodeLabels[ [from, to] ][2] = gain;
				nodeLabels[ [from, to] ][3] = phase;
				}
			{ (gain <= -90) && nodes[ [frombus, tobus] ].notNil } {
	...
	getValues {
		...
		dict.add( \nodes		-> nodeLabels )  // statt busnums je in und out: device-name und channel, sowie gain, phase und active
		^dict;
	}
	...
	setValues { arg dict;
		...
		if (dict.includesKey( \nodes )) { dict[\nodes].keysValuesDo({ arg key, array, i;
			var from, to, gain, phase;
			from = inLabels2.indexOfEqual( array[0] ); // -1 ??
			to = outLabels2.indexOfEqual( array[1] ); // -1 ??
			if ( from.notNil && to.notNil ) { 
				gain = array[2];
				phase = array[3];
				this.setNodeByIndex(from, to, gain, phase);
				MXGUI.matrixView.grid.setState_( from, to, (gain > -90).if( { gain.dbamp }, 0.0 ), phase );  // schnelle unsaubere Notloesung ....
			}
		})};
	}
