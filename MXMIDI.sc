	...
	*new { arg messageType, channel, number, uid, midiout, bank, slot, type, displayValueFunc;
	...
	init { arg argmessageType, argchannel, argnumber, arguid, argmidiout, argbank, argslot, argtype, argdisplayValueFunc;
		...
		if ( [\noteOn, \noteOff, \polytouch, \control].includes(messageType) ) {
			midiinfunc = {Êarg src, chan, num, val;
		...
		if ( messageType == \bend ) {
			midiinfunc = {Êarg src, chan, val;
			 	// [\bend, src, chan, val, uid, channel].postln;
				if ( (bank == MXMIDI.midiDevice.currentBank) || (bank.isNil) ) {
					if ( (src == uid) && (chan == channel) ) {
						value = val * (MXGlobals.midiControllerName == \QCon).if( MXMIDIQCon.iconbend.reciprocal, 16383.reciprocal );
	...
	send {
		var list, sendval;
		if ( messageType == \bend ) {
			sendval = (MXGlobals.midiControllerName == \QCon).if( { value * MXMIDIQCon.iconbend }, { value * (2**14)  } );
			list = [channel, sendval.asInteger];
		} {	
			if ( [\noteOn, \noteOff, \polytouch, \control].includes(messageType) ) {
...
MXMIDIQCon {
	classvar <iconbend = 14847; // pitchbendrange bei QCon leider nicht genau 14 bit!! (-8192 ... 6655)

	var <deviceName = "iCON Qcon Ver1.02" ;
	var <portInName = "iCON Qcon Ver1.02";
	var <portOutName = "iCON Qcon Ver1.02";
	var <endpoint1, <uid1;
	var <midiout1;
	var <endpoint2, <uid2;
	var <midiout2;
...
/*
LCD: 2 Zeilen je 64 Zeichen, offset fuer untere Zeile = 64 !
...
*/
	...
	*new {
	...
	init {
		var bank = 0;
		endpoint1 = MIDIIn.findPort(deviceName, portInName);
		midiout1 = MIDIOut.findPort(deviceName, portOutName);
		if (endpoint1.isNil) { (deviceName + "MIDI in port" +  portInName + "not found!").warn; } { uid1 = endpoint1.uid; } ;
		if (midiout1.isNil) { (deviceName + "MIDI out port" +  portOutName + "not found!").warn; };
		if ( endpoint1.notNil && midiout1.notNil ) {
			midiout1 = MIDIOut.newByName(deviceName, portOutName);	
			"Initializing QCon ...".postln;
			midiout1.sysex(Int8Array[ 16rf0, 0, 0, 16r66, 16r14, 16r62,  16rF7]); // all LEDs off
			midiout1.sysex(Int8Array[ 16rf0, 0, 0, 16r66, 16r14, 16r0C, 1, 16rF7]); // touchless
			midiout1.sysex(Int8Array[ 16rf0, 0, 0, 16r66, 16r14, 16r61, 1, 16rF7]); // Faders min
			midiout1.sysex(Int8Array[ 16rf0, 0, 0, 16r66, 16r14, 16r0A, 0, 16rF7]); // no click
			midiout1.sysex(Int8Array[ 16rf0, 0, 0, 16r66, 16r14, 16r0B, 1, 16rF7]); // back light off after 1 min

			this.clearLCD;

		 	midiout1.sysex( Int8Array.newFrom([ 16rf0, 0, 0, 16r66, 16r14, 16r12, 0] 
		 		++ "           Elektronisches Studio der TU Berlin                        Fachgebiet Audiokommunikation".ascii ++ [16rF7]) ); 
		 	
		 	(16r76).do { arg i; midiout1.noteOn(0, i , 0) };
			9.do({ arg n; midiout1.bend(n, 0)});
			...
			// ------------------------
			bank = nil;
			// 									message, channel, number, uid, out, bank, slot, type, displayfunc
//			modelMessageDict.add( \monitorvolume0 -> [\bend, 8, nil, uid1, midiout1, bank, nil, nil, {Êarg slot, value; this.displayLED1(value) } ] );
			modelMessageDict.add( \monitorvolume0 -> [\bend, 8, nil, uid1, midiout1, bank, 8 ] );
//			modelMessageDict.add( \monitormute0 -> [\noteOn, 0, 16r5F, uid1, midiout1, bank, nil, \toggle] );
			modelMessageDict.add( \monitormute0 -> [\noteOn, 0, 48, uid1, midiout1, bank, nil, \toggle] );
//			modelMessageDict.add( \srdisplay -> [nil, nil, nil, uid1, midiout1, bank, nil, nil, {Êarg slot, value; this.displayLED2("SR: " ++ value.asInteger.asString) } ] );
			modelMessageDict.add( \srdisplay -> [nil, nil, nil, uid1, midiout1, bank, nil, nil, {Êarg slot, value; /* noch LCD !!! */ } ] );

			bank = 0;
			8.do {Êarg i;
				modelMessageDict.add( ("monitorvolume" ++ (i+1).asString).asSymbol  -> [\bend, i, nil, uid1, midiout1, bank, i] );
			//	modelMessageDict.add( ("monitormute" ++ (i+2).asString).asSymbol  -> [\noteOn, 0, i + 4 + 16r10, uid1, midiout1, bank,nil, \toggle] );   // MUTE buttons
				modelMessageDict.add( ("monitoron" ++ (i+1).asString).asSymbol  -> [\noteOn, 0, i + 16r08, uid1, midiout1, bank, nil, \toggle] );  // SOLO buttons
			};

			// ------------------------
			// bank 1: main channels controls:
			// ------------------------
			bank = 1;
			8.do {Êarg i;
				modelMessageDict.add( ("mainchannelgain" ++ (i+1).asString).asSymbol -> [\bend, i, nil, uid1, midiout1, bank, i] );  // Faders
			...
			//	modelMessageDict.add( ("mainchannelphase" ++ (i+1).asString).asSymbol -> [\noteOn, 0, i + 16r0, uid1, midiout1, bank, nil, \toggle] );  // RECs
				modelMessageDict.add( ("mainchannelon" ++ (i+1).asString).asSymbol -> [\noteOn, 0, i + 16r08, uid1, midiout1, bank, nil, \toggle] );  // SOLOs
				modelMessageDict.add( ("mainchannelphase" ++ (i+1).asString).asSymbol -> [\noteOn, 0, i + 16r10, uid1, midiout1, bank, nil, \toggle] );  // MUTEs
			...
			// ------------------------
			// bank 2: device controls:
			// ------------------------
			bank = 2;
			8.do {Êarg i;
				modelMessageDict.add( ("devicevolume" ++ (i+1).asString).asSymbol -> [\bend, i, nil, uid1, midiout1, bank, i] );
			//	modelMessageDict.add( ("devicemute" ++ (i+1).asString).asSymbol -> [\noteOn, 0, i + 16r10, uid1, midiout1, bank, nil, \toggle] );
			//	modelMessageDict.add( ("deviceon" ++ (i+1).asString).asSymbol -> [\noteOn, 0, i + 16r08, uid1, midiout1, bank, nil, \toggle] );
				modelMessageDict.add( ("devicemute" ++ (i+1).asString).asSymbol -> [\noteOn, 0, i + 0, uid1, midiout1, bank, nil, \toggle] );
			};
...
/*	
	displayLED1 {Êarg value;  // ASSIGNMENT Display
		var string;
		case 
	...
	displayLED2 {Êarg value;	// TimeCode Display
	...
	//	("displayDeviceName " ++ slot + name ).postln;
		if (slot.notNil) {
			...
			{ slot.isNumber } {
				...
				if (slot < 8) {
					shift = slot * 7;
//					midiout1.sysex( Int8Array.newFrom([ 16rf0, 0, 0, 16r66, 21, 16r12, shift, name[0..6].ascii, 16rF7].flat ) ); 
					midiout1.sysex( Int8Array.newFrom([ 16rf0, 0, 0, 16r66, 16r14, 16r12, shift, name[0..6].ascii, 16rF7].flat ) ); 
				} 
			}
			;
		}
	...
	displayModelValue { arg slot, value;
		...
		if (slot.notNil) {
			...
			{ slot.isNumber } {
				if (slot < 9) {
					shift = slot * 7;
			//		midiout1.sysex( Int8Array.newFrom( [ 16rf0, 0, 0, 16r66, 21, 16r12, 56 + shift] ++ 32 
					midiout1.sysex( Int8Array.newFrom( [ 16rf0, 0, 0, 16r66, 16r14, 16r12, 56 + shift] ++ 32 
	...
	clearUpperLCD {
		midiout1.sysex( Int8Array.newFrom([ 16rf0, 0, 0, 16r66, 16r14, 16r12, 0, 32.dup(56), 16rF7].flat ) ); 
	 	}

	clearLowerLCD {
		midiout1.sysex( Int8Array.newFrom([ 16rf0, 0, 0, 16r66, 16r14, 16r12, 56, 32.dup(56 + 8 + 8), 16rF7].flat ) ); 
		}
	...
	bankChange { // reset all elements ! 	
		...
		(0..8).do {Êarg i;  midiout1.bend(i, 0); };

		(0..7).do {Êarg i; midiout1.noteOn(0, 0 + i, 0); }; // rec
		(0..7).do {Êarg i; midiout1.noteOn(0, 8 + i, 0); }; // solo
		(0..7).do {Êarg i; midiout1.noteOn(0, 16 + i, 0); }; // mute
		(0..7).do {Êarg i; midiout1.noteOn(0, 24 + i, 0); }; // sel
		(0..7).do {Êarg i; midiout1.noteOn(0, 32 + i, 0); }; // ??
			
		MXMIDI.midiModels.do {Êarg model, i; model.displayName }; 
	}	
}
	...
	*new {
	...
	//		midiout1.sysex( Int8Array.newFrom([ 16rf0, 0, 0, 16r66, 16r10, 16r12, 0, {arg i; "  L " ++ (i+9).asPaddedString(2," ", \right) ++ " "}.dup(4).ascii, 16rF7].flat ) ); 
		...
		//	midiout2.sysex( Int8Array.newFrom([ 16rf0, 0, 0, 16r66, 21, 16r12, 0, 32.dup(112), 16rF7].flat ) ); // clear LCD
			...
			bank = nil;
			modelMessageDict.add( \monitorvolume1 -> [\bend, 8, nil, uid1, midiout1, bank, nil, nil, {Êarg slot, value; this.displayLED1(value) } ] );
...
*/	
	...
	*connectControlToMIDI { arg cv, modelName, deviceName, inverted = false;
		...
		if (model.notNil ) {
			...
			scout = SimpleController(cv).put(\synch, {Êarg changer, what; // \midi
				var value;
				value = changer.input;
