...
MXMatrixGUI - 
	compositeview für Set-Verwaltung (new, clear, select, merge ...)
	compositeview für selected Node 
	compositeview für Grid
	
MXMatrixGrid - View/Editor für Matrizen, zeigt jeweils nur eine Matrix

MXMatrixBox - View für einen Matrixknoten
...
MXMatrixGUI {
	...
	var <inDeviceName, <outDeviceName, <inNum, <outNum, <inBus, <outBus, <valNum, <valSlider, <phaseButton;
	var <level, <phase;
...
//		selRow = MXCV(ControlSpec(0, matrix.fromBusArray.size - 1, \lin, 1), 0);  
		selRow = MXCV(ControlSpec(0, matrix.toBusArray.size - 1, \lin, 1), 0);  
...
//		selCol = MXCV(ControlSpec(0, matrix.toBusArray.size - 1, \lin, 1), 0);  
		selCol = MXCV(ControlSpec(0, matrix.fromBusArray.size - 1, \lin, 1), 0);  
		...
		//	matrix.setNodeByIndex(selRow.value, selCol.value, level.value);
			matrix.setNodeByIndex(selCol.value, selRow.value, level.value, phase.value);
			if(testtoggle.value > 0) { this.settestsynth };
		...
		phase.action_( { arg changer, what; 
		//	matrix.setNodeByIndex(selRow.value, selCol.value, level.value);
			matrix.setNodeByIndex(selCol.value, selRow.value, level.value, phase.value);
			if(testtoggle.value > 0) { this.settestsynth };
...
*/
	...
	makeNodeView {
		...
		//	.states_([["mute", Color.white(1, 0.5), Color.red(1, 0.5) ], ["mute", Color.white, Color.red(1)]])
			...
			.action_({ arg view;
				if ( view.value == 1 ) { matrix.mute } { matrix.unmute };
			});
	...
	//		.borderColor_(Color.grey(0.5))
		...
		phaseButton = MXButton(nodeview, (buttonSize.x * 0.5) @ buttonSize.y, radius )
			.shifty_(shifty)
			.border_(border)
			.borderColor_(Color.grey(0.5))
			.font_(nodeFont)
		//	.states_([[ "Test", MXGUI.plugColor, Color.black], [ "Test",  Color.black, MXGUI.plugColor]])
			.states_([[ "-180°",  Color.grey(0), MXGUI.plugColor], [ "-180°", Color.grey(0), Color.grey(0.7)]])
			.connect(phase)
			;
...
//	var <transposed=false, <showinlabels=true, <showoutlabels=true;
	...
	*new { arg parent, bounds, inArray, outArray; 
	...
	setState_ {arg col, row, state, phase;
		//if(state.isInteger, {state = state!=0});
		gridNodes[col][row].setState_(state, phase);
		matrixView.refresh;
	}
...
MXMatrixGridNode {
	...
	*new { arg rect, column, row, color ; 
	...
	setState_ {arg argstate, argphase;
		state = argstate;
		phase = argphase;
	}
