...
Rücklaufzeit: 1.7 s von 0 bis -20 dB  (0 .. -60 db -> 5.1 s  == 12 db/s ??
*/
MXMeterBar {  // View
	...
	*new { arg parent, bounds; 
	...
	//	bar = SCUserView(bview, Rect(0, clipsize + clipgap, barwidth, barheight )) //(parent, barbounds)
		bar = SCUserView(bview, Rect(4, clipsize + clipgap, barwidth - 8, barheight )) //(parent, barbounds)
			...
			.drawFunc_({ arg view;
				var height, width, smallwidth, left=3; // left=4;
...
MXAudioMeterView {
	...
	var <barwidth, <width, <numberheight =20, <gap=6; // <gap=10
	var <barsbounds;
	...
	//	barsbounds = bounds.top_(bounds.top + (2 * numberheight)).height_(bound.height - (2 * numberheight));
		...
		peakString = Array.fill( size, { arg i; 
			SCStaticText(view, Rect( (barwidth+gap) * (i+1), 0, barwidth+5, numberheight) )
	...
	*new { arg name, type, numChannels;
	...
	//					vals = (x.ampdb + magscale).linlin(dbmin,dbmax,0,1);
				...
				//		MXMeterManager.rta.guiUpdateFunc.value( vals[2 .. 303] ); // ??????
						MXMeterManager.sona.guiUpdateFunc.value( vals );
...
/*
- vergibt Inputbusse für "meter devices"
...
*/
	...
	*init {
		devices = List.new;
		multiMeter = MXMultiChannelMeter(numChannels: MXGlobals.multiMeterChannels);
		rta = MXRTA.new;
		fft = MXFFT.new;
		sona = MXSonagram.new;
		...
		analyserRequests.action = { arg changer; 
			if (changer.value > 0) { 
				if(rta.notNil) { rta.active_(1) };
				if(fft.notNil) { fft.active_(1) };
				if(sona.notNil) { sona.active_(1) };
			} {
				if(rta.notNil) { rta.active_(0) };
				if(fft.notNil) { fft.active_(0) };
				if(sona.notNil) { sona.active_(0) };
			};
		};
...
*/	
	...
	*registerAnalyserRequests { arg bool;
		if (bool) { analyserRequests.inc } { analyserRequests.dec };
	}
	
	*registerMultiMeterRequests { arg bool;
		if (bool) { multiMeterRequests.inc } { multiMeterRequests.dec };
	}
