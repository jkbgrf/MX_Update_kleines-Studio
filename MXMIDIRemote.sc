...
// class for up to 8 bits of a MIDI sysex data byte
	...
	setBits { arg value, lsb, numbits;
		...
		bitfield = value.asInteger.asBinaryString.reverse.collectAs({Êarg char;  char.digit.booleanValue }, Array);
		(value.asInteger).max(1).numBits.do {Êarg i;  newvalue  = newvalue.setBit(lsb + i, bitfield[i]) };
		cv.value = newvalue;
	}
	...
	init { arg argdeviceID;
		...
		if ( endpoint.notNil && midiout.notNil  ) {
			...
			8.do { arg i;  // Channels Gain coarse
				...
				remotevalue.midioutfunc = {Êarg cvinput;
		...
		//		remotebyte.cv = MXCV(ControlSpec(0, 127, \lin, 1), 0);
				...
				remotebyte.midioutfunc = {Êarg cvinput;
					midiout.sysex(header ++ Int8Array[0x20, parambyte, cvinput, eox] );
				};
		...
		//		remotebyte.cv = MXCV(ControlSpec(0, 127, \lin, 1), 0);
				...
				["input", "Hi Z", "Autoset", "Lo Cut", "M/S", "Phase", "48V"].do {Êarg name, b;
			...
			1.do { arg i; // Setup 1
				...
				["int freq", "clockspeed", "clocksource", "outdBref"].do {Êarg name, b;
			...
			1.do { arg i; // Setup 2
				...
				["lock", "peakhold", "followclock", "autosetlimit", "delaycomp", "autodevice"].do {Êarg name, b;
					paramname = name.asSymbol;
					if (b == 3) {
						parambit = 3;
						remotevalue = MXMIDIRemoteBits(paramname, parambit, 2, remotebyte);
			...
			1.do { arg i; // Lock / Sync
				...
				["optionlock", "optionsync", "aeslock", "aessync", "wcklock", "wcksync", "wcout"].do {Êarg name, b;
					paramname = name.asSymbol;
					parambit = b;
					remotevalue = MXMIDIRemoteBits(paramname, parambit, 1, remotebyte);
					valuesDict.add( paramname -> remotevalue );
...
//		bounds = parent.bounds;
		...
		channelPanels = 8.collect {Êarg i;
			var p, view;
