...
//	var <meters;			// MXSimpleMeter
	...
	*new { arg device, num, in, out, active=0;
	...
	delay_ { arg value;
		if ( ((value > 0) && (delay.value == 0))  ||� ((value == 0) && (delay.value > 0)) ) {
			this.removeSynth;
			delay.value = value;
			this.makeSynth;
		} {
...
//	var <target;			// Parent Group Node
	...
	var <wfs; 			// MXCV: [0, 1]  activates WFS monitoring
	var <dome; 			// MXCV: [0, 1]  activates Dome / Ambisonics monitoring
	...
	*new { arg name, type, ioDict;
	...
	init { arg argname, argtype, argioDict;
		...
		monitorConnections.add( \wfs -> nil );
		monitorConnections.add( \dome -> nil );
...
*/
		...
		active.action = { arg changer, what;
			...
			if (changer.value == 1) {
				switch (type)
					{ \in } 	{�this.startDSP(MXMain.deviceManager.inGroup) }
					{ \madibridgeIN } 	{�this.startDSP(MXMain.deviceManager.inGroup) }  					{ \out } 	{�this.startDSP(MXMain.deviceManager.outGroup) }
					;
			} {
				this.stopDSP;
...
//		this.makeChannels;
	...
	setSR {	// to be called by the device manager when sampleRate is known and server has been booted
		...
		switch (type)
			{ \in } 	{�	inputs = ioArray;  		// Array of MXInputs
						outputs = busArray; 	// Array of MXBusses
					}
			{ \madibridgeIN } 	{�	inputs = ioArray; 	// Array of MXInputs  added by jg
						outputs = busArray; 	     // Array of MXBusses
					}
			{ \out } 	{�	inputs = busArray; 	// Array of MXBusses
		...
		this.makeSynthDef;
	//	this.makeChannels;
	//	if (type ==  \in) {�meter = MXSimpleMeter(this, busArray.collect(_.busNum)) };
...
/*
	makeChannels {
		...
		// make busArray for current samplerate
		busArray = ioDict[ srspeed ].collect {�arg io, i;  MXDeviceManager.getNewBus };

		// make an inactive MXDeviceChannel for each MXIO !
		switch (type)
			{ \in } 	{�channels = ioDict[ srspeed ].collect {�arg io, i; MXDeviceChannel(this, i, io.busNum, busArray[i].busNum, active.value) } }
			{ \out } 	{�channels = busArray.collect {�arg mxbus, i; MXDeviceChannel(this, i, mxbus.busNum, ioDict[ srspeed ][i].busNum, active.value) } }
			;
	}
...
//			sig = ins.collect { arg in; In.ar(in, 1) };
	...
	getValues {
		...
		dict.add( \near 		-> near.value );
		dict.add( \wfs 		-> wfs.value );
		dict.add( \dome 		-> dome.value );
	...
	setValues { arg dict;
		...
		if (dict.includesKey( \near )) 		{ this.near_(dict[\near]); };
		if (dict.includesKey( \wfs )) 		{ this.wfs_(dict[\wfs]); };
...
/*		nearButton = MXButton(panel, button2size, radius)
			...
			.font_(font);
*/
		wfsButton = MXButton(panel, button3size, radius)
			.shifty_(shifty)
			.states_([  ["WFS", buttonTextColorOff, buttonBackColorOff],
					   ["WFS", buttonTextColorOn, Color.green] ])
			.font_(font);

		domeButton = MXButton(panel, button3size, radius)
			.shifty_(shifty)
			.states_([  ["Dome", buttonTextColorOff, buttonBackColorOff],
					   ["Dome", buttonTextColorOn, Color.green] ])
			.font_(font);

		phonesButton = MXButton(panel, button3size, radius)
			.shifty_(shifty)
	...
	//	nearButton.connect(device.near);
		...
		device.routSV.action = { arg changer, what;
			if ( changer.value > 0 ) {
				{�routingMenu.background = Color.green; }.defer;
			} {
				{�routingMenu.background = buttonBackColorOff;}.defer;
			}
		};
		...
		device.controlSV.action = { arg changer, what;
			if ( changer.value > 0 ) {
				{�controlMenu.background = Color.yellow;  }.defer;
			} {
				{�controlMenu.background = buttonBackColorOff; }.defer;
			}
		};
...
// abfrage ob madi bridge vorhanden
			if (dict.includesKey( \madibridgeIN )) {
				dict[\madibridgeIN].do {�arg assoc, i;
	...
	*addDeviceFromArray { arg type, array;
		...
		latency = array[5].asFloat;  // delay in ms !!!
		sampleRates = [44100.0, 48000.0, 88200.0, 96000.0];
		busNums = Dictionary.new;
		busNums.add( 48000.0 -> array[1].clump(2).collect({ arg a; (a[0]..a[1]) }).flat.collect(_.asInteger - 1) );
...
//		busNums.add( 96000.0 -> (( array[2][0] .. array[2][1] ) ).collect(_.asInteger - 1) );
	...
	*setSR { // to be called once sampleRate is known and server has been booted
		...
		inDevices.do {�arg d; d.setSR };
		outDevices.do {�arg d; d.setSR };
	}

	*unsetSR { // to be called when sampleRate will be changed and before server reboots
		inDevices.do {�arg d; d.unsetSR };
		outDevices.do {�arg d; d.unsetSR };
	}
	...
	*startDSP { arg inTarget, outTarget;
		...
		inGroup = inTarget;
		outGroup = outTarget;
		inDevices.do {�arg d;   d.active = 1 };
		outDevices.do {�arg d;   d.active = 1 };
	//	outDevices.do {�arg d;   d.startDSP(outGroup) };
	}

	*stopDSP {  // ??
		inDevices.do {�arg d; d.stopDSP };
		outDevices.do {�arg d; d.stopDSP };
		{ inGroup.free; outGroup.free }.defer( MXGlobals.switchrate + 0.01 );
	}

	*reset {
		inDevices.do {�arg d;   d.reset };
		outDevices.do {�arg d;   d.reset };
	}
