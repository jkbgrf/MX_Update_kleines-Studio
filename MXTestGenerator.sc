MXTestGenerator {
	...
	var <bufc;			// Buffer of click soundfile
	var <pathc;			// path to click soundfile
	var <synth;
	...
	*initClass {		
		...
		MXGlobals.synthdefs.add(
			SynthDef("test_click", { arg out=1, rate=1, amp=1, on=1, gate=1, buf=0;				var sig, trig;
				trig = Impulse.ar(rate);
			//	sig = Blip.ar(30, 300, amp);
			//	sig = WhiteNoise.ar(amp);
			//	sig = Decay.ar(trig, 0.2, sig ); 
			//	sig = Decay2.ar(trig, 0.003, 0.2, sig ); 
			//	sig = LeakDC.ar(sig);
				sig = PlayBuf.ar(1, buf, BufRateScale.ir(buf), trig, 0, 0, 0);
				sig = sig * Linen.kr(gate, MXGlobals.switchrate, 1, MXGlobals.switchrate, doneAction: 2);
				sig = sig * on;
				Out.ar(out, sig);
			}, [0, 0.01, MXGlobals.levelrate, MXGlobals.switchrate, 0, 0] );
		);
		MXGlobals.synthdefs.add(
...
*/
		signalSV = MXSV([\off, \sine, \square, \saw, \impulse, \click, \greynoise, \clipnoise, \brownnoise, 'pinknoise', 'whitenoise', \soundfile], \off);
		signalSV.action = { arg changer, what;
			this.setmode;
		};		

		path = Platform.resourceDir +/+ "sounds/a11wlk01.wav";
		pathc = MXGlobals.configDir ++ "../sounds/testclick.wav";
	...
	loadSoundfile {Êarg pathname;
		path = pathname ? path;
		if (buf.notNil) { buf.free };
		buf = Buffer.readChannel(MXGlobals.server, path, channels: [0] , action: { arg buf; synth.set(\buf, buf.bufnum) });
	}

	loadClickfile {Êarg pathname;
		pathc = pathname ? pathc;
	//	if (bufc.notNil) { bufc.free };
		bufc = Buffer.readChannel(MXGlobals.server, pathc, channels: [0]);
	}
	...
	setmode {
		...
		switch(signalSV.item)
			...
			{\click}		{	synth = Synth.controls("test_" ++ signalSV.item,
							 [out: outputNums[0], rate: 1, buf: bufc.bufnum, amp: [level, level.dbamp]], group);
					}
	...
	startcycle {
		...
		cycletask = Task({
			...
			inf.do {
				...
				inMatrix.nodes.sortedKeysValuesDo( {Êarg array, node;
					if(node.notNil) { node.unmute };
					cycletime.value.wait;
					if(node.notNil) { node.mute };
				}, sortfunc );
				outMatrix.nodes.sortedKeysValuesDo( {Êarg array, node;
					if(node.notNil) { node.unmute };
					cycletime.value.wait;
					if(node.notNil) { node.mute };
				}, sortfunc );
