...
// parameters provided by XML file:
	...
	var <>delayFlag = true;	 // Boolean: if false no latency compensation here
...
*/
	...
	startDSP { arg target;
		...
		// start only synths for interfaces with gain != 0 or phase != 1 or delay != 0 !!
		if ( (gain != 0.0) || (phase != 1) || (latency > 0) ) {
			if ( (latency > 0) && delayFlag ) { 
				synths = SynthDef(name.asString + type.asString + "_delay", { // arg gain=1.0, gate=1;
					var sig, bus, dt;
					bus = busNums[ MXGlobals.srspeeds[ MXGlobals.sampleRate ] ];
					sig = In.ar(bus);
					dt = MXStatus.srCV.value.reciprocal * latency;
					sig = DelayN.ar(sig, dt, dt );
				//	sig = DelayN.ar(sig, 0.01, latency * SampleDur.ir );
					sig = sig * gain.dbamp * phase; // * Linen.kr(gate, MXGlobals.switchrate, 1, MXGlobals.switchrate);
					bus.do { arg out, i; ReplaceOut.ar(out, sig[i]) };
					// ReplaceOut.ar(bus, sig);
				} /*, [\ir, 0]*/ ).play(group /* ,[ \gain, gain.dbamp * phase ]*/);
			} {
				synths = SynthDef(name.asString + type.asString, { // arg gain=1.0, gate=1;
					var sig, bus;
					bus = busNums[ MXGlobals.srspeeds[ MXGlobals.sampleRate ] ];
					sig = In.ar(bus);
					sig = sig * gain.dbamp * phase; // * Linen.kr(gate, MXGlobals.switchrate, 1, MXGlobals.switchrate);
...
//	classvar <inputs;	     		// List of MXIOs 
	...
	*init {
		MXGlobals.synthdefs.add (
			SynthDef("out_swap_node", { arg gate=1;
				var sig0, sig128, env;
				env = Linen.kr(gate, MXGlobals.switchrate, 1, MXGlobals.switchrate, doneAction: 2);
			//	env = 1.0;
				sig0 = In.ar(0, 128) * env;
				sig128 = In.ar(128, 128) * env;
			//	ReplaceOut.ar(0, sig128 ++ sig0);  // <-- Server crashes !!! too many channels for ReplaceOut ?
			 	ReplaceOut.ar(0, sig128);
				ReplaceOut.ar(128, sig0);
			}));	
		MXGlobals.synthdefs.add (
			SynthDef("in_swap_node", { arg gate=1;
				var sig0, sig128, env;
	//			var shift = MXGlobals.numOutputs;
				var shift = 320;
				env = Linen.kr(gate, MXGlobals.switchrate, 1, MXGlobals.switchrate, doneAction: 2);
		//		env = 1.0;
				sig0 = In.ar(MXGlobals.numOutputs, 128) * env;
				sig128 = In.ar(MXGlobals.numOutputs + 128, 128) * env;
				ReplaceOut.ar(MXGlobals.numOutputs, sig128 ++ sig0); // <-- Server doesn't crash !!!
			//	ReplaceOut.ar(128, sig0);
			}));
		...
		//	arrayfromfile.postln;
			...
			if (dict.includesKey( \inputs )) { 
				dict[\inputs].do {Êarg assoc, i;
					...
					//this.addInterfaceFromArray(\madibridgeIN, [assoc.key] ++ assoc.value ); // added by Jakob Greif
				}
			} { 
	...
	*addInterfaceFromArray { arg type, array;
		...
		*/
		...
		interface.delayFlag = nodelay.not;
		...
		switch (type)
		 	...
		 	// {\madibridgeIN} 	{ inInterfaces.add(interface) }  //added by Jakob Greif
		 	{\out} 	{ outInterfaces.add(interface) }
		;
	}
	
	*calculateDelays {
		var minLatency, maxLatency, diffLatency;
		minLatency = inInterfaces[0].latency;
		maxLatency = inInterfaces[0].latency;
		inInterfaces.do({ arg interface; 
			if (interface.latency < minLatency) { minLatency = interface.latency };  // not needed ?
			if (interface.latency > maxLatency) { maxLatency = interface.latency };
		});
		inInterfaces.do({ arg interface; interface.latency = maxLatency - interface.latency });
		
		minLatency = outInterfaces[0].latency;
		maxLatency = outInterfaces[0].latency;
		outInterfaces.do({ arg interface; 
			if (interface.latency < minLatency) { minLatency = interface.latency };  // not needed ?
			if (interface.latency > maxLatency) { maxLatency = interface.latency };
		});
		outInterfaces.do({ arg interface; interface.latency = maxLatency - interface.latency });
	}
	...
	//	srspeed = MXGlobals.srspeeds[ MXGlobals.sampleRate ];
		[48000.0, 96000.0].do({ arg sr, i;
			...
			inInterfaces.do {Êarg d, i; 
				...
				/*
				d.busNums[ sr ].do { arg busNum, b;
					mxio = MXIO(\madibridgeIN, busNum, inputsCounter, d.name, b+1);
					inputsCounter = inputsCounter + 1;
					inputs[sr].add(mxio);
					inputsOrder[sr][busNum] = mxio;
					// "Input added".postln;
	...
	*startDSP { arg inTarget, outTarget; 
		...
		inInterfaces.do {Êarg d;  d.startDSP(inGroup) };
		outInterfaces.do {Êarg d;  d.startDSP(outGroup) };
	}
	
	*stopDSP {  
		"IOManager-stopDSP".postln;
		inInterfaces.do {Êarg d;   d.stopDSP };
		outInterfaces.do {Êarg d;   d.stopDSP };
