...
MXGlobals { // singleton !
	classvar <>audioDeviceName; 
	classvar <midiControllerName;
...
//	classvar <>sampleRateOnBoot; // compare regularly with current sr, for warning about sr clock change ...
	...
	*initClass {
		...
		audioDeviceName = "MADI+DANTE"; 
	//	midiControllerName = \NanoKontrol; 
		midiControllerName = \QCon; // \MackieControl16; // \MackieControl8, \QCon, \Tascam2400, \BCF2000

		numAudioBusses = 2048+128;
		numInputs = 256+64; //56;
		numOutputs = 256+64; //56;
		//numAux = 8;
		//numListen = 2;
		sampleRate = defaultsampleRate;
	//	blockSize = defaultblockSize;
		blockSize = 256;  // was: 128
	//	hardwareBufferSize	= defaulthardwareBufferSize;
		hardwareBufferSize	= 512;
		...
		multiMeterChannels	= 24;  // number of channels in multimeterview
...
//		meterrms = 100;	// number of audio samples for integration
		...
		fftdisplayinterval = 20.reciprocal; // 25.reciprocal;
	...
	*setServer {	
		server = Server.local;
		server.options.memoryLocking = true; // !!!!!!!
		...
		server.options.memSize = 8192 * 8;
		server.options.numWireBufs = 256 * 8;
		server.options.maxNodes = maxNodes;
		server.options.maxSynthDefs = 1024 * 8;
		server.latency = 0.2;
