	...
	*initClass {		
		...
		MXGlobals.synthdefs.add (
			SynthDef(defNameDelay, { arg in=0, out=1, gain=0.0, delay=0.0, on=1, gate=1;			var sig;
				sig = In.ar(in, 1);
				sig = LeakDC.ar(sig);
				sig = DelayN.ar(sig, MXGlobals.delaymax, delay);
				sig = sig * gain * Linen.kr(gate, MXGlobals.switchrate, 1, MXGlobals.switchrate, doneAction: 2);
				sig = sig * on;
				Out.ar(out, sig);
	...
	*new { arg device, num, in, out, active=0;
	...
	makeSynth {
		if (device.function == \sub) { // sub gain is controlled also by main gain !
			if (((delay.value + device.delayoffset) > 0.0) && (device.delayOn.value > 0)) {
...
//	var <target;			// Parent Group Node
	...
	var <isActive = false;	// hack gegen mehrfache Aktivierung per update
//	var <mute;			// MXCV: [0, 1]  (overwrites channels)
	...
	var <delayOn;			// MCV: delay activation [0, 1] (for lateny compensation)
...
//	var <masterSynth;	// Synth (Node) for Monitor volume, limiter, DCkiller, denormals handling etc  
	...
	function_ { arg symbol;
		...
		switch (function)
			...
			{ \dome }		{ MXMonitorManager.domeMonitor = this }
		;
	}
	...
	*new { arg name, type, ioDict;
	...
	//	mute = MXCV( ControlSpec(0, 1, \lin, 1), 0);   // not used !
		active = MXCV( ControlSpec(0, 1, \lin, 1), 0);  // called by GUI on/off button and MonitorManager
		active.action = { arg changer, what; 
			if (changer.value == 1) {
				if (isActive.not) {
					(name.asString ++ " activated!").postln;
					this.startDSP(MXMain.monitorManager.group);
					isActive = true;
				}
			} {
				if (isActive) {
					(name.asString ++ " de-activated!").postln;
					this.stopDSP;
					isActive = false;
				}
			};	
		...
		delayOn = MXCV( ControlSpec(0, 1, \lin, 1), 0);  // called by GUI on/off button and MonitorManager
		delayOn.action = { arg changer, what; 
			if ((delay.value + delayoffset) > 0) {
				if (changer.value == 1) {
					if (isActive) {
						this.active_(0);
						{ this.active_(1) }.defer( (2*MXGlobals.switchrate) + 0.01);
					}
				} {
...
//		this.makeChannels;
	...
	makeChannels {
		...
		channels = inputNums.collect {Êarg bus, i; MXMonitorChannel(this, i, bus, outputNums[i], active.value) };
	}
	...
	makeMultiDef { // for monitordevices with more than 12 channels (Dome, WFS ...)
		SynthDef(name + "_multi", { arg gain=0.0, gate=1;			var sig;
			sig = In.ar(inputNums, 1);
			sig = LeakDC.ar(sig);
			sig = sig * gain * Linen.kr(gate, MXGlobals.switchrate, 1, MXGlobals.switchrate, doneAction: 2);
		//	sig = sig * on;
			outputNums.collect({arg out, i; Out.ar(out, sig[i]); });
		}, [MXGlobals.levelrate, 0] ).add(\global);
		
		SynthDef(name + "_multi_delay", { arg gain=0.0, delay=0.0, gate=1;			var sig;
			sig = In.ar(inputNums, 1);
			sig = LeakDC.ar(sig);
			sig = DelayN.ar(sig, MXGlobals.delaymax, delay);
			sig = sig * gain * Linen.kr(gate, MXGlobals.switchrate, 1, MXGlobals.switchrate, doneAction: 2);
		//	sig = sig * on;
			outputNums.collect({arg out, i; Out.ar(out, sig[i]); });
		}, [MXGlobals.levelrate, MXGlobals.delayrate, 0] ).add(\global);
	}
	
	makeMultiSynth {
		if (((delay.value + delayoffset) > 0.0) && (delayOn.value > 0)) {
			multiSynth = Synth.controls(name + "_multi_delay", 
				[	
					gain: [ [gain, phase], (gain + gainoffset).dbamp * (phase * phaseoffset) ],
					delay: [ delay, (delay + delayoffset) * 0.001]
				], group, \addToHead);
		} {
			multiSynth = Synth.controls(name + "_multi", 
					[	
				gain: [ [gain, phase], (gain + gainoffset).dbamp * (phase * phaseoffset) ]
				], group, \addToHead);
		};
	...
	removeMultiSynth {
		multiSynth.set(\gate, 0);
	}
	
		
	makeMonoSynth {
		monoSynth = SynthDef(name + "_mono", { arg gate=1;			var sig, mono;
	...
	makeLimiterSynth {
		limiterSynth = SynthDef(name + "_limiter", { arg gate=1;			var sig;
	...
	makeCrossoverSynth {
		...
		crossoverSynth = SynthDef(name + "_crossover", { arg gate=1;			var sig;
			sig = In.ar(outputNums);
			if (function == \sub) {
...
*/
	...
	//	target = target;
		...
		if (inputs.size <= 12) {
			{ channels.do {Êarg chan;  chan.active_(1) } }.defer( MXGlobals.switchrate + 0.01 );
		} {
			{ this.makeMultiSynth }.defer( MXGlobals.switchrate + 0.01 );
		};
...
*/
	...
	classvar <>domeMonitor; 	// MXMonitorDevice, Dome / Ambisonics monitor device

	*init {
		devices = List.new;
	}
		
	*addDevice {Êarg device;
		devices.add(device);
	}
	
	*setSR {
		"MonitorManager-setSR".postln;		
		devices.do {Êarg d; d.setSR };	
	}
	
	*unsetSR {
		devices.do {Êarg d; d.unsetSR };	
	}
		
	*startDSP { arg target; 
		"MonitorManager-startDSP".postln;
		group = target;
	//	devices.do {Êarg d;   d.active_(1) };
		devices.do {Êarg d;   if(d.active.value < 1) { d.active_(1) }; };
	...
	*stopDSP {  // ??
		...
		devices.do {Êarg d;   d.active_(0) };  // schon getestet ?
	...
	*reset {
		devices.do {Êarg d;   d.reset };
	}	

	*connectMainMonitorToMIDI {
//	MXMIDI.connectControlToMIDI(gain, ("devicevolume" ++ changer.value.asString).asSymbol);
		MXMIDI.connectControlToMIDI(mainMonitor.gain, \monitorvolume0, mainMonitor.name );
		MXMIDI.connectControlToMIDI(mainMonitor.active, \monitormute0, mainMonitor.name, inverted: true);
	...
	//		MXMIDI.connectControlToMIDI(ch.on, ("mainchannelmute" ++ i.asString).asSymbol, "Spkr" ++ i, inverted: true);
			MXMIDI.connectControlToMIDI(ch.on, ("mainchannelon" ++ i.asString).asSymbol, "Spkr" ++ i, inverted: false);
			MXMIDI.connectControlToMIDI(ch.phase, ("mainchannelphase" ++ i.asString).asSymbol, "Spkr" ++ i, inverted: true);
		};
	}

	*connectMonitorsToMIDI {
		MXMIDI.connectControlToMIDI(wfsMonitor.gain, \monitorvolume5, ' WFS');
		MXMIDI.connectControlToMIDI(wfsMonitor.active, \monitoron5, wfsMonitor.name, inverted: false);
	//	MXMIDI.connectControlToMIDI(wfsMonitor.active, \monitormute2, wfsMonitor.name, inverted: true);
		MXMIDI.connectControlToMIDI(domeMonitor.gain, \monitorvolume6, ' Dome');
		MXMIDI.connectControlToMIDI(domeMonitor.active, \monitoron6, domeMonitor.name, inverted: false);
	//	MXMIDI.connectControlToMIDI(domeMonitor.active, \monitormute3, domeMonitor.name, inverted: true);
	...
	//	MXMIDI.connectControlToMIDI(nearMonitor.active, \monitormute3, nearMonitor.name, inverted: true);
		MXMIDI.connectControlToMIDI(phonesMonitor.gain, \monitorvolume7, \Phones);
		MXMIDI.connectControlToMIDI(phonesMonitor.active, \monitoron7, phonesMonitor.name, inverted: false);
	//	MXMIDI.connectControlToMIDI(phonesMonitor.active, \monitormute4, phonesMonitor.name, inverted: true);
		MXMIDI.connectControlToMIDI(subMonitor.gain, \monitorvolume8, ' Sub');
		MXMIDI.connectControlToMIDI(subMonitor.active, \monitoron8, subMonitor.name, inverted: false);
	...
	*disconnectFromMIDI {
		...
		mainMonitor.channels.do { arg ch, i;
			...
			MXMIDI.disconnectControlFromMIDI(ch.phase);
		};
		MXMIDI.disconnectControlFromMIDI(wfsMonitor.gain);
		MXMIDI.disconnectControlFromMIDI(wfsMonitor.active);
		MXMIDI.disconnectControlFromMIDI(domeMonitor.gain);
		MXMIDI.disconnectControlFromMIDI(domeMonitor.active);
